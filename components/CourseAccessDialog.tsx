...
export default function CourseAccessDialog({
  ...
  useEffect(() => {
    if (accessMode !== "free" || telegramUser || !telegramBotUsername) return;
  ...
  }, [accessMode, telegramUser, telegramBotUsername, telegramWidgetAttempt]);
  ...
  async function verifyTelegramAccess() {
    ...
    try {
      const response = await fetch("/api/courses/access", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ courseId: course.id, telegramUser }),
      });
      ...
      <div className="surface w-full max-w-lg border p-6 shadow-2xl sm:p-8">
        <div className="flex items-start justify-between gap-5">
          <div>
            ...
            <h2 id="course-dialog-title" className="mt-2 text-2xl font-black">
              {course.title}
            </h2>
          </div>
          <button
            ...
            <div className="mt-6 grid gap-3" role="tabpanel">
              ...
              >
                <span>
                  ...
                  {formatCoursePrice(course)} with Flutterwave
                </span>
